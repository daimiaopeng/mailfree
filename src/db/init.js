...
async function createIndexes(db) {
  ...
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_audit_logs_created ON audit_logs(created_at DESC);`);
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_audit_logs_action ON audit_logs(action, created_at DESC);`);
}
...
async function migrateMailboxesFields(db) {
  try {
    ...
    if (!columnNames.includes('note')) {
      await db.exec("ALTER TABLE mailboxes ADD COLUMN note TEXT DEFAULT '';");
    }
    if (!columnNames.includes('tags')) {
      await db.exec("ALTER TABLE mailboxes ADD COLUMN tags TEXT DEFAULT '';");
    }
    if (!columnNames.includes('purpose')) {
      await db.exec("ALTER TABLE mailboxes ADD COLUMN purpose TEXT DEFAULT '';");
    }
  } catch (error) {
